...
class Payment_for_ShareAdmin(admin.ModelAdmin):
    list_display = ['client_name', 'amount', 'paid', 'timestamp', 'payment_purpose', 'payment_ref' ]
admin.site.register(Payment_for_Share, Payment_for_ShareAdmin)

class Withdrawal_RequestAdmin(admin.ModelAdmin):
    list_display = ['client_name', 'share_id', 'account_name', 'account_number', 'bank_name', 'timestamp']
